...
// ESP32 connects to your laptop hotspot
...
void setup() {
    ...
    Serial.println("🚀 ESP32 Bridge System Starting...");
    
    // First, connect ESP32 to your laptop hotspot
    connectToHotspot();
    
    // Then create ESP32's own network for students/lecturers
    setupAccessPoint();
    ...
    Serial.println("✅ ESP32 Bridge System Ready!");
    Serial.println("📱 Students/Lecturers connect to ESP32 WiFi");
...
<head>
    ...
    <title>🚪 ESP32 Bridge - Attendance System</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="bridge-info">
            <h3>🔗 Bridge Mode Active</h3>
            <p>You're connected to the ESP32 bridge. The ESP32 forwards your requests to the Django server.</p>
            <p><strong>Bridge:</strong> )" + String(DEVICE_NAME) + R"(</p>
...
bool sendAttendanceToDjango(String studentId, String courseCode, String deviceId) {
    ...
    Serial.println("📤 Sending to Django through ESP32 bridge: " + jsonString);
...
void handleStatus() {
    String status = "{\"device_name\":\"" + String(DEVICE_NAME) + "\",\"connected_devices\":" + String(deviceCount) + ",\"hotspot_connected\":" + (WiFi.status() == WL_CONNECTED ? "true" : "false") + ",\"status\":\"active\"}";
...
void updateConnectedDevices() {
    ...
    if (WiFi.softAPgetStationNum() > deviceCount) {
        Serial.println("📱 New device connected to ESP32 bridge");
